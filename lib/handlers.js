...
function handleCreateFeatureService(req, res) {
  ...
  var p = portals[token];
  if (p !== undefined) {
    newFS(p, name, folderId);
  } else {
    p = new Portal(token);

    p.on('load', function(portal) {
      portals[token] = portal;
      newFS(portal, name, folderId);
    });
  ...
  function newFS (portal, fsName, folderId) {
    console.log('Creating feature service ' + fsName + ' in folder ' + folderId);
    return portal.createGnipFeatureService(name, folderId)
